...
`scripts/bench.sh` 测量 eebpf 运行时的四项开销（CPU / 内存 / 时延 / 吞吐），结果输出至 `report/benchmark.md`。
...
| 指标 | 基线 (无 eebpf) | 加载 eebpf | 变化 |
|------|-----------------|-----------|------|
| 系统 CPU | 56% | 54% | ±2%（噪声范围内） |
| 进程 RSS | — | 2.8 MB | 极低 |
| BPF map 锁定内存 | — | 14.8 MB | — |
| I/O P99 时延 | 1 μs | 1 μs | 0% |
| IOPS | 408671 | 410160 | +0.3% |

详细测量结果见 `report/benchmark.md`。
